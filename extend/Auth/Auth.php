...
class Auth extends Controller
{
 ...
 //   protected static $instance;
    ...
    {
        //可设置配置项 auth, 此配置项为数组。
        if ($auth = Env::get('auth')) {
            $this->config = array_merge($this->config, $auth);
        }
        // 初始化request
     //  $this->request = $request;// 5.18 取消
    }
    /**
     * 初始化
     * @access public
    ...
    public function check($name, $uid, $relation = 'or', $type = 1, $mode = 'url')
    {
        ...
        if ('url' == $mode) {
          // $REQUEST = unserialize(strtolower(serialize($this->request->param())));
            $REQUEST = unserialize(strtolower(serialize(Request::param()))); //5.1.8更改
    ...
    public function getGroups($uid)
    {
        ...
        // 转换表名
        $type = Env::get('database.prefix') ? 1 : 0;
